...
RPM_DISTROS=${RPM_DISTROS:-Fedora:34 Fedora:34:arm64 Fedora:35 Fedora:35:arm64 Fedora:36 CentOS:7 CentOS:8 CentOS:8:arm64 almalinux:8.6 CentOS:stream8 CentOS:stream9}
...
for DISTRO in $DISTROS; do
	...
	echo "$DISTRO" | egrep -iv "fedora|centos|alma|rocky" >& /dev/null
	RPM="$?"
	if [ "${RPM}" == "1" ]; then
		LIB="/usr/lib64"
